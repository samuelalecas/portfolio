...
const WeatherApp = () => {
  ...
  const {data, hasError, isFetching, fetchData} = useFetch<Weather>({url: `https://api.weatherapi.com/v1/forecast.json?key=42da7e4d019546aaa4d82110231307&q=${location}&days=5&aqi=no&alerts=no&lang=es`})
    ...
    <Header/>
      <div className={`flex flex-col w-screen h-screen justify-center items-center bg-gray-900`}>
        <div className="bg-gray-700 text-white rounded-md p-8 text-center relative pt-12 mb-12">
          ...
          <div>
            <SearchBar location={location} onSubmit={fetchData} onChange={(e) => setLocation(e.target.value)}/>
          </div>
        </div>
